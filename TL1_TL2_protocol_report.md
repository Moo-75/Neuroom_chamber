...
| Choice window | TL1: user-entered seconds, default 20 s; TL2: 10 s | Left cue blinks near the lower-left area. A valid choice is detected on the left sensor rising edge, when the nose enters the port. The event, sound cue, and target change start immediately without waiting for nose withdrawal. Other pokes are ignored. |
